...
const About = () => {
  ...
  const findAll = async () => {
    const doc_refs = await getDocs(collection(firestore, "shoes"))
    const res: { id: string; }[] = []
    doc_refs.forEach(shoes => {
        res.push({
            id: shoes.id,
            ...shoes.data()
        }) 
      ...
      {shoes.map((shoe: any) => 
        ...
        <div className="description p-6 md:basis-1/2 md:py-[40px]">
          ...
          <div className="price flex items-center">
            <span className="text-3xl font-[700] mr-4">{shoe.price} €</span>
          ...
          <div className="buttons-container flex flex-col md:flex-row mt-8">
            <div className="state w-[100%] flex justify-around md:justify-center items-center space-x-10 bg-lightGrayishBlue rounded-lg p-3 md:p-2 md:mr-4 md:w-[150px]">
              ...
              <p className="md:text-[14px] font-bold">{qty} ///// {shoe.id}</p>
              <button
